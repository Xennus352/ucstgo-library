...
function notifyIssue(row) {
  ...
  try {
    ...
    logger.error({ err: err?.message }, "[monitor] socket notify error");
  }
}

function persistIssue(row) {
  return pool
    .query(
    ...
    .then((res) => {
      if (res.rowCount > 0) {
        notifyIssue({ ...row, id: res.rows[0].id, count: res.rows[0].count });
        return;
      }
      ...
      return pool
        ...
        .then(() => notifyIssue({ ...row, id, count: 1 }));
    });
}
...
function flushIssueBuffer() {
  ...
  Promise.all(batch.map((row) => persistIssue(row))).catch(() => {
    issueBuffer.unshift(...batch);
  });
}

pool.on("error", (err) => {
  logger.error({ err: err?.message }, "[monitor] DB pool error");
  logIssue({
    source: "db",
    severity: "critical",
...
async function refreshBlockedIps() {
  try {
    ...
    logger.error({ err: err?.message }, "[monitor] Failed to refresh blocked IPs");
    logIssue({
      source: "db",
      severity: "warning",
...
async function runDailyCleanup() {
  try {
    ...
    logger.info("[monitor] Daily log cleanup completed");
  } catch (err) {
    logger.error({ err: err?.message }, "[monitor] Log cleanup failed");
    logIssue({
      source: "db",
      severity: "warning",
...
function trackRequest(req) {
  try {
    ...
    logger.error({ err: err?.message }, "[monitor] trackRequest error");
  }
}
